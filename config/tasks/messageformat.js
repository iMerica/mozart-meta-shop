...
module.exports = function (grunt) {
    ...
    grunt.registerMultiTask("messageformat", "Compiles MessageFormat JSON objects to JS", function () {
        var locales = {};
        grunt.file.recurse(this.file.src, function scanPath (abspath, rootdir, subdir, filename) {
        ...
        Object.keys(locales).forEach(function(locale) {
            ...
            grunt.log.write("Writing " + locale + " locale to " + grunt.task.current.file.dest + "/" + locale + ".js...");
            grunt.file.write(
                path.resolve(grunt.task.current.file.dest, locale + ".js"),
                compiledString
            );
            grunt.log.ok();
        });
